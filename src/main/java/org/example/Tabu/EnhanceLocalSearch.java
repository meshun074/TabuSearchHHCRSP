...
import static org.example.Tabu.EvaluationFunction.UpdateCost;

public class EnhanceLocalSearch implements Runnable {
    @Override
    public void run() {
        tabuSearch.getLSSolution().add(localSearch());
    }
    private TabuSearchMain tabuSearch;
    private final Set<Integer> tabu;
    private Solution ch;
    private final Random rand;
    private static Patient[] allPatients;
    private static int allCaregivers;
    private static double[][] distances;
    public EnhanceLocalSearch( TabuSearchMain tabuSearch, Solution ch, Queue<Integer> tabu) {
    ...
    private Solution localSearch() {
        ...
        int size = (int) (patientLength * (0.7+0.2*Math.random()));
